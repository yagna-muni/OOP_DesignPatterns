...
public class PrimeGenerator {
    ...
    public PrimeGenerator(long from, long to){
        if(from >= 1 && to > from){ // checks if from value is less than to value, else throws exception
            this.from = from;
            this.to = to;
        }else{
            throw new RuntimeException("Wrong input values: from=" + from + " to=" + to);  // throws a Runtime exception
        }
    }

    public LinkedList<Long> getPrimes(){ return primes; };  // when this method is called, it returns primes List (no print)
